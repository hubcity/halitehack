async function renderer({
  ...
  const info = state.map((player, player_index) => {
    const next_step = environment.steps[step+1];
    if (next_step) {
      const pinfo = next_step[player_index].info;
  ...
  const rects = {
    ship: [
      ...
      "6,12;5,13,3;4,14,5;13,12;11,14,5;12,13,3",
      "8,8,4;9,7,2;7,9,2;9.5,6;11,9,2"
    ],
    flame: [
  ...
  if (!parent.querySelector("#buffer")) {
    ...
    // Cargo Ships.
    colors.cargo_ships.forEach((color, n) => {
      for (let d in directions) {
        move(ctx, { x: 500 + 100 * n, y: 400 + d * 100, angle: d * 90 }, () => {
          rects.ship.forEach((v, i) => drawRects(ctx, v, color[i], 5));
        });
      }
    });
  ...
  players.forEach((player, player_index) => {
    Object.entries(player[2]).forEach(([uid, [pos, cargo]]) => {
      ...
      if (cargo == 0) {
        ...
        fgCtx.drawImage(bufferCanvas, shipx, sy + 400, ss, ss, dx, dy, ds, ds);
      }
  ...
  players.forEach((player, player_index) => {
    ...
    fgCtx.fillStyle = colors.players[player_index];
    fgCtx.font = "normal 14px sans-serif";
    if (player_index == 0) {
      fgCtx.textBaseline = "top";
      fgCtx.textAlign = "left";
      fgCtx.fillText("".concat(player[0]), 0.5 * cellSize, 1.5 * cellSize);
      fgCtx.fillText("".concat(info[player_index]), 0.5 * cellSize, 0.5 * cellSize);
    } else if (player_index == 1) {
      fgCtx.textBaseline = "top";
      fgCtx.textAlign = "right";
      fgCtx.fillText("".concat(player[0]), width - 0.5 * cellSize, 1.5 * cellSize);
      fgCtx.fillText("".concat(info[player_index]), width - 0.5 * cellSize, 0.5 * cellSize);
    } else if (player_index == 2) {
      fgCtx.textBaseline = "bottom";
      fgCtx.textAlign = "left";
      fgCtx.fillText("".concat(player[0]), 0.5 * cellSize, height - 1.5 * cellSize);
      fgCtx.fillText("".concat(info[player_index]), 0.5 * cellSize, height - 0.5 * cellSize);
    } else if (player_index == 3) {
      fgCtx.textBaseline = "bottom";
      fgCtx.textAlign = "right";
      fgCtx.fillText("".concat(player[0]), width - 0.5 * cellSize, height - 1.5 * cellSize);
      fgCtx.fillText("".concat(info[player_index]), width - 0.5 * cellSize, height - 0.5 * cellSize);
    }
  });
}
